  ...
  return (
    <div>
      ...
      <div className="card mb-4 shadow-sm border-0">
        <div className="card-body">
          <div className="d-flex justify-content-between align-items-start">
            ...
            {toQuestionHref && (
              <a className="btn btn-sm btn-outline-primary" href={toQuestionHref}>View on Question Page</a>
            )}
          </div>
          <div className="mt-2 fw-semibold">{qTitle}</div>
          <div className="mt-2 text-muted" style={{ whiteSpace: "pre-wrap" }}>{qBody}</div>
      ...
      <div className="card shadow-sm border-0">
        <div className="card-body">
          <pre className="mb-0" style={{ whiteSpace: "pre-wrap" }}>{attempt.content || "(empty)"}</pre>
        </div>
      </div>
    </div>
  );
