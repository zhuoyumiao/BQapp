...
export default function QuestionForm({ initial, onSubmit, submitting }) {
  ...
  useEffect(() => {
    ...
    setCompany(Array.isArray(initial?.company) ? initial.company.join(", ") : (initial?.company || ""));
  }, [initial]);
  ...
  const submit = (e) => {
    ...
    onSubmit({
      ...
      company: company.split(",").map(s => s.trim()).filter(Boolean),
    });
  };
  ...
  return (
    <form onSubmit={submit} className="vstack gap-3">
      ...
      <div>
        ...
        <textarea className="form-control" rows="6" value={body} onChange={e => setBody(e.target.value)} required />
      </div>
      ...
      <div className="d-flex gap-2">
        ...
        <a className="btn btn-outline-secondary" href="#/admin/questions">Cancel</a>
      </div>
    </form>
  );
}
